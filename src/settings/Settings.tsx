...
// 导航：归一组 + 特色功能 + 系统
// 归一：账号管理（GLM+Codex）/ 预警（GLM+Codex）
// 特色：空转（GLM 专属）/ 鉴权分发（Codex 专属）
// 系统：通用 / 主题 / 导出 / 关于
type NavId =
  | "accounts"
  | "spin"
...
const navGroups: NavGroup[] = [
  ...
  },
  {
    ...
    items: ["general", "theme", "export", "about"],
  },
];
    ...
    case "codex":
      return (
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
          ...
          <path d="M7 10l-2 2 2 2M17 10l2 2-2 2M12 8l-1 8" />
        </svg>
      );
    case "general":
      return (
...
const titleMap: Record<NavId, string> = {
...
export default function Settings({ onBack, screenHeight }: { onBack: () => void; screenHeight: number }) {
  ...
  return (
    ...
    >
      {/* Sidebar */}
      <nav className="w-[80px] bg-[var(--color-bg-secondary)] border-r border-[var(--color-border)] flex flex-col py-3 px-2 shrink-0">
        <button
          onClick={onBack}
        ...
        <div className="flex-1 scroll-area space-y-3">
          {navGroups.map((group) => (
            <div key={group.labelKey} className="space-y-1">
              {/* 分组小标题 */}
              <div className="text-[8px] font-semibold text-[var(--color-text-tertiary)]/60 uppercase tracking-wider text-center px-1 pt-1 pb-0.5">
                {t(group.labelKey)}
              </div>
              {group.items.map((id) => (
                <button
        ...
        <div className="text-[9px] text-[var(--color-text-tertiary)] text-center font-medium tabular-nums pt-2">v{APP_VERSION}</div>
      </nav>
