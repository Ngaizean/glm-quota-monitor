...
export default function AboutPane() {
  ...
  async function handleCheckUpdate() {
    ...
    try {
      ...
      // 区分错误类型：
      // 1. release JSON 不存在/无效 → 无更新源（latest.json 还没上传）
      // 2. 网络连接失败 → 网络错误
      // 3. 其他 → 通用失败
      if (msg.includes("release json") || msg.includes("could not fetch") || msg.includes("404") || msg.includes("not found")) {
        setStatus({ kind: "noSource" });
      } else if (msg.includes("network") || msg.includes("connection") || msg.includes("timeout") || msg.includes("fetch") || msg.includes("dns")) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-10 space-y-4">
      ...
      <div className="flex flex-col items-center gap-2 w-full max-w-[220px]">
        {/* 检查更新按钮（idle / checking / upToDate / error / noSource 时显示） */}
        {(status.kind === "idle" || status.kind === "checking" || status.kind === "upToDate" || status.kind === "error" || status.kind === "noSource") && (
          <>
            <button
            ...
            {status.kind === "noSource" && (
              <>
                <span className="text-[10px] text-[var(--color-warning)] text-center leading-relaxed">
                  {t("aboutPane.updateSourceUnavailable")}
                </span>
                <button
                  onClick={() => openExternal(GITHUB_RELEASES_URL)}
                  className="text-[10px] font-medium text-[var(--color-accent)] hover:text-[var(--color-accent-hover)] transition-[var(--transition-fast)] px-2.5 py-1 rounded-md hover:bg-[var(--color-accent-subtle)]"
                >
